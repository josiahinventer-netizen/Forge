...
const forgeServer = createForgeServer({ databasePath, host, port, allowedOrigin });
const address = await forgeServer.start();
console.log(`Forge Sync Server listening on http://${address.address}:${address.port}`);
console.log(`Database: ${databasePath}`);
console.log('LAN access is disabled until local HTTPS and device pairing are configured.');
